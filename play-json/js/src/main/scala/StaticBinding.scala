...
object StaticBinding {
  ...
  private def fromJs(jsValue: JsValue, escapeNonASCII: Boolean, ilevel: Int, indent: Int => String, newline: Boolean = false, fieldValueSep: String = ":", arraySep: (String, String, String) = ("[", ",", "]")): String = {
    def str = jsValue match {
      ...
      case JsArray(items) => {
        ...
        items.map(fromJs(_, escapeNonASCII, il, indent,
          newline, fieldValueSep, arraySep)).
      ...
      case JsObject(fields) => {
        ...
        fields.map {
          case (k, v) =>
            ...
            @inline def value = fromJs(v, escapeNonASCII, il, indent,
              newline, fieldValueSep, arraySep)
  ...
  @inline private def fromString(s: String, escapeNonASCII: Boolean): String = if (!escapeNonASCII) JSON.stringify(s) else escapeStr(JSON.stringify(s))
