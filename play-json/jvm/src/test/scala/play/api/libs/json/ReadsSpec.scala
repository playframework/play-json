...
class ReadsSpec extends org.specs2.mutable.Specification {
  ...
  "Local date/time" should {
    ...
    "not be read from invalid string" in {
      reads(JsString("invalid"))
        .aka("read date")
        .must(beLike {
          case JsError((_, JsonValidationError("error.expected.date.isoformat" :: Nil, _) :: Nil) :: Nil) => ok
        })
    }
  ...
  "Local time" should {
    ...
    "be successfully read from number" in {
      ...
      reads(JsNumber(BigDecimal.valueOf(d.toNanoOfDay))).aka("read time").must_==(JsSuccess(d))
    }
  ...
  "Instant" should {
    ...
    "be successfully read from number" in {
      reads(JsNumber(JBigDec.valueOf(123L))).aka("read date").must_==(JsSuccess(Instant.ofEpochMilli(123L)))
    }
    ...
    "be successfully read with default implicit" >> {
      "from '2015-05-01T13:00:00Z' (with zeros)" in {
        reads(JsString("2015-05-01T00:00:00Z"))
          .aka("read data")
          .must_==(JsSuccess(Instant.parse("2015-05-01T00:00:00Z")))
      }
  ...
  "ZoneId" should {
    ...
    "not be read from number" in {
      ...
      reads1.must(beLike {
        case JsError((_, JsonValidationError("error.expected.jsstring" :: Nil) :: Nil) :: Nil) => ok
      })
    }

    "not be read from unknown time zone" in {
      reads(JsString("America/Gotham")).must(beLike {
        case JsError((_, JsonValidationError("error.expected.timezone" :: Nil, "America/Gotham") :: Nil) :: Nil) => ok
      })
    }

    "not be read from malformed time zone" in {
      reads(JsString("UTC+x")).must(beLike {
        case JsError((_, JsonValidationError("error.expected.timezone" :: Nil, "UTC+x") :: Nil) :: Nil) => ok
      })
    }
  }
