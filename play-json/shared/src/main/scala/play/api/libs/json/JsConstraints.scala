...
trait ConstraintFormat {
  ...
  @inline def of[A](implicit
      fmt: Format[A]
  ): Format[A] = fmt

  def optionWithNull[A](implicit
      fmt: Format[A]
  ): Format[Option[A]] =
...
trait PathFormat {
  ...
  def at[A](path: JsPath)(implicit
      f: Format[A]
  ): OFormat[A] =
    OFormat[A](Reads.at(path)(f), Writes.at(path)(f))

  def withDefault[A](path: JsPath, defaultValue: => A)(implicit
      f: Format[A]
  ): OFormat[A] =
    OFormat[A](Reads.withDefault(path, defaultValue)(f), Writes.at(path)(f))

  def nullable[A](path: JsPath)(implicit
      f: Format[A]
  ): OFormat[Option[A]] =
    OFormat(Reads.nullable(path)(f), Writes.nullable(path)(f))

  def nullableWithDefault[A](path: JsPath, defaultValue: => Option[A])(implicit
      f: Format[A]
  ): OFormat[Option[A]] =
...
trait PathReads {
  ...
  def required(path: JsPath)(implicit
      reads: Reads[JsValue]
  ): Reads[JsValue] = at(path)(reads)
  ...
  def jsPick[A <: JsValue](path: JsPath)(implicit
      reads: Reads[A]
  ): Reads[A] = at(path)(reads)
...
trait ConstraintReads {
  ...
  def optionWithNull[T](implicit
      rds: Reads[T]
  ): Reads[Option[T]] = Reads {
  ...
  def list[A](implicit
      reads: Reads[A]
  ): Reads[List[A]] = Reads.traversableReads[List, A]

  def set[A](implicit
      reads: Reads[A]
  ): Reads[Set[A]] = Reads.traversableReads[Set, A]

  def seq[A](implicit
      reads: Reads[A]
  ): Reads[Seq[A]] = Reads.traversableReads[Seq, A]

  def map[A](implicit
      reads: Reads[A]
  ): Reads[collection.immutable.Map[String, A]] = Reads.mapReads[A]

  /**
   * Defines a minimum value for a Reads. Combine with `max` using `andKeep`, e.g.
   * `.read(Reads.min(0) andKeep Reads.max(100))`.
   */
  def min[O](m: O)(implicit
      reads: Reads[O],
      ord: Ordering[O]
  ) =
   ...
   */
  def max[O](m: O)(implicit
      reads: Reads[O],
      ord: Ordering[O]
  ) =
    filterNot[O](JsonValidationError("error.max", m))(ord.gt(_, m))(reads)

  def filterNot[A](error: JsonValidationError)(p: A => Boolean)(implicit
      reads: Reads[A]
  ) =
    Reads[A](js => reads.reads(js).filterNot(JsError(error))(p))

  def filter[A](otherwise: JsonValidationError)(p: A => Boolean)(implicit
      reads: Reads[A]
  ) =
  ...
  def minLength[M](m: Int)(implicit
      reads: Reads[M],
      p: M => scala.collection.Iterable[_]
  ) =
   ...
   */
  def pattern(regex: => scala.util.matching.Regex, error: String = "error.pattern")(implicit
      reads: Reads[String]
  ) =
    Reads[String](js =>
      reads.reads(js).flatMap { o => regex.unapplySeq(o).map(_ => JsSuccess(o)).getOrElse(JsError(error)) }
    )
  ...
  def verifying[A](cond: A => Boolean)(implicit
      rds: Reads[A]
  ) =
...
trait ConstraintWrites {
  ...
  def pruned[A](implicit
      w: Writes[A]
  ): Writes[A] = new Writes[A] {
  ...
  def list[A](implicit
      writes: Writes[A]
  ): Writes[List[A]] = Writes.iterableWrites2[A, List[A]]

  def set[A](implicit
      writes: Writes[A]
  ): Writes[Set[A]] = Writes.iterableWrites2[A, Set[A]]

  def seq[A](implicit
      writes: Writes[A]
  ): Writes[Seq[A]] = Writes.iterableWrites2[A, Seq[A]]

  def map[A](implicit
      writes: Writes[A]
  ): OWrites[Map[String, A]] = Writes.genericMapWrites[A, Map]
