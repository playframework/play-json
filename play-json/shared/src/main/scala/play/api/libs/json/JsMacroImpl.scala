  ...
  private def macroImpl[A, M[_], N[_]](
    ...
    class ImplicitResolver(resolvedType: Type => Type) {
      ...
      private class ImplicitTransformer[T](
        ...
        private def denormalized(ptype: Type): Type =
          ptype match {
    ...
    class CaseClass[T](tpeArgs: List[Type])(implicit tag: WeakTypeTag[T]) {
      ...
      @inline private def params: List[(Name, Type)] =
        applyFunction match {
          case Some((_, _, ps, _)) => {
    ...
    def directKnownSubclasses: Option[List[Type]] = {
      ...
      @annotation.tailrec
      def allSubclasses(path: Traversable[Symbol], subclasses: Set[Type]): Set[Type] =
