...
object JsMacroImpl {
  ...
  inline def withOptionsReadsImpl[A](using inline A: Mirror.Of[A]): Expr[Reads[A]] =
    macroImpl[A, Reads, Reads](withOptionsConfig, "read", "map", reads = true, writes = false)

  inline def withOptionsWritesImpl[A](using inline A: Mirror.Of[A]): Expr[OWrites[A]] =
    macroImpl[A, OWrites, Writes](withOptionsConfig, "write", "contramap", reads = false, writes = true)

  inline def withOptionsFormatImpl[A](using inline A: Mirror.Of[A]): Expr[OFormat[A]] =
    macroImpl[A, OFormat, Format](withOptionsConfig, "format", "inmap", reads = true, writes = true)

  inline def implicitConfigReadsImpl[A](using m: Mirror.Of[A]): Expr[Reads[A]] =
    macroImpl[A, Reads, Reads](implicitOptionsConfig, "read", "map", reads = true, writes = false)

  inline def implicitConfigValueReads[A](using inline A: Mirror.Of[A]): Expr[Reads[A]] =
    valueImpl[A, Reads](implicitOptionsConfig, "read")

  inline def implicitConfigWritesImpl[A](using inline A: Mirror.Of[A]): Expr[OWrites[A]] =
    macroImpl[A, OWrites, Writes](implicitOptionsConfig, "write", "contramap", reads = false, writes = true)

  inline def implicitConfigValueWrites[A](using inline A: Mirror.Of[A]): Expr[Writes[A]] =
    valueImpl[A, Writes](implicitOptionsConfig, "write")

  inline def implicitConfigFormatImpl[A](using inline A: Mirror.Of[A]): Expr[OFormat[A]] =
    macroImpl[A, OFormat, Format](implicitOptionsConfig, "format", "inmap", reads = true, writes = true)

  inline def implicitConfigValueFormat[A](using inline A: Mirror.Of[A]): Expr[Format[A]] =
  ...
  inline protected def readsImpl[A](using inline A: Mirror.Of[A]): Expr[Reads[A]] =
    macroImpl[A, Reads, Reads](implicitOptionsConfig, "read", "map", reads = true, writes = false)

  @deprecated("Use implicitConfigWritesImpl or withOptionsWritesImpl", "2.6.6")
  inline protected def writesImpl[A](using inline A: Mirror.Of[A]): Expr[OWrites[A]] =
    macroImpl[A, OWrites, Writes](implicitOptionsConfig, "write", "contramap", reads = false, writes = true)

  @deprecated("Use implicitConfigFormatImpl or withOptionsFormatImpl", "2.6.6")
  inline protected def formatImpl[A](using inline A: Mirror.Of[A]): Expr[OFormat[A]] =
    macroImpl[A, OFormat, Format](implicitOptionsConfig, "format", "inmap", reads = true, writes = true)

  private def withOptionsConfig: Expr[JsonConfiguration] =
    ??? //Expr[JsonConfiguration](c.typecheck(q"${c.prefix}.config"))

  private def implicitOptionsConfig: Expr[JsonConfiguration] =
    ??? //Expr[JsonConfiguration](c.inferImplicitValue(c.typeOf[JsonConfiguration]))

  inline private def valueImpl[A, M[_]](
      config: Expr[JsonConfiguration],
      methodName: String
  )(using inline A: Mirror.Of[A]): Expr[M[A]] = {
...
//    }
   ...
   */
  inline private def macroImpl[A, M[_], N[_]](
      config: Expr[JsonConfiguration],
      methodName: String,
...
//
//    def macroSealedFamilyImpl(subTypes: List[Type]): Expr[M[A]] = {
//      if (subTypes.isEmpty) {
...
//    }
//
//    def macroCaseImpl(tpeArgs: List[Type]): Expr[M[A]] = {
