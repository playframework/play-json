  ...
  "JSON transformers " should {
    ...
    "report the correct path in the JsError" when {
      "the field to modify doesn't exist" in {
        ...
        ).asEither.swap.getOrElse(sys.error("unexpected right")).head mustEqual ((__ \ Symbol("field42"), Seq(JsonValidationError("error.path.missing"))))
      }

      "the reader is the wrong type" in {
        js.transform(
          (__ \ Symbol("field2")).json.update(__.read[JsString])
        ).asEither.swap.getOrElse(sys.error("unexpected right")).head mustEqual ((__ \ Symbol("field2"), Seq(JsonValidationError("error.expected.jsstring"))))
      }
    }
  }
}
