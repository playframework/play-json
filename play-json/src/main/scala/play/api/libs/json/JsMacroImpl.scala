...
object JsMacroImpl {
  ...
  private def macroImpl[A, M[_], N[_]](c: blackbox.Context, methodName: String, mapLikeMethod: String, reads: Boolean, writes: Boolean)(implicit atag: c.WeakTypeTag[A], matag: c.WeakTypeTag[M[A]], natag: c.WeakTypeTag[N[A]]): c.Expr[M[A]] = {
    ...
    class ApplyUnapply[T](tpeArgs: List[Type])(implicit tag: WeakTypeTag[T]) {
      // Common definitions
    ...
    class ImplicitResolver[T](
      tpeArgs: List[Type],
      tparams: List[TypeSymbol],
      applyUnapply: ApplyUnapply[T]
    ) {
    ...
    def macroCaseImpl: c.Expr[M[A]] = {
      ...
      val applyUnapply = new ApplyUnapply[A](tpeArgs)
      val (companioned, isCase) = applyUnapply.validCaseClass
      ...
      val resolver = new ImplicitResolver(tpeArgs, tparams, applyUnapply)
      val resolvedImplicits = resolver(params)
      val canBuild = resolvedImplicits.map {
        case Implicit(name, pt, impl, _, _) =>
